...
if (!form.pricePerKg)
    e.pricePerKg = "Required";

if (!form.availableQuantityKg)
    e.availableQuantityKg = "Required";
    return e;
  };
  ...
  const handleSave = async () => {
    ...
    availableQuantityKg: parseFloat(form.availableQuantityKg),
      ...
      <div style={{ background:"#161616", border:"1px solid #2a2a2a", borderRadius:20, padding:40, width:580, maxWidth:"100%", maxHeight:"90vh", overflowY:"auto", position:"relative" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            {field("variety",   "Rice Variety")}
            {field("qualityGrade",  "Quality Grade", "text", { placeholder:"e.g. Grade A, Organic, Premium" })}
            {field("origin", "Origin", "text", { placeholder:"e.g. Haryana, Punjab" })}
            {field("pricePerKg", "Price per KG (₹)", "number", { min:1, step:0.01 })}
           {field("availableQuantityKg","Available Quantity (KG)","number",{min:0,step:1})}
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#1a1a1a", border:"1px solid #333", borderRadius:8, padding:"14px 16px" }}>
            ...
            <div onClick={() => setForm(p => ({...p, available:!p.active}))}
...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label:"Available Products",value:products.filter(p=>p.active).length,       icon:"✅", color:"#22c55e" },
      ...
      <div style={{ background:"#161616", borderBottom:"1px solid #242424", padding:"0 40px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:64 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            ...
            />
            <div>
              <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:18, color:"#f0f0f0" }}>MBRG</div>
              ...
              <div className="fade-in">
                ...
                {products.length === 0 ? (
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:16 }}>
                    {products.map(p => (
                      <div key={p.id} className="row-card" style={{ borderLeft:`3px solid ${p.active ? "#22c55e" : "#444"}` }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                          <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                            <div style={{ fontSize:32 }}>{p.icon || "🌾"}</div>
                            <div>
                              <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:18, color:"#f0f0f0" }}>{p.variety}</div>
                              <div style={{ fontSize:12, color:"#666", marginTop:2 }}>{p.origin} · {p.qualityGrade}</div>
                            </div>
                          </div>
                          <span className="badge" style={{ background: p.active ? "#22c55e22" : "#44444422", color: p.active ? "#22c55e" : "#888", border:`1px solid ${p.active ? "#22c55e44" : "#44444444"}` }}>
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", borderTop:"1px solid #242424", paddingTop:14 }}>
                          <div>
                            ...
                            <div style={{ fontSize:11, color:"#666", marginTop:2 }}>Stock: {p.stockQuantityMT} + " Kg"</div>
                          </div>
                          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                            {/* Toggle availability */}
                            <button className="toggle" onClick={() => toggleAvailability(p)}
                              style={{ background: p.active ? "#22c55e" : "#444" }}
                              title={p.active ? "Click to hide from website" : "Click to show on website"}>
                              <div style={{ width:18, height:18, background:"#fff", borderRadius:"50%", position:"absolute", top:3, left: p.active ? 23 : 3, transition:"left 0.3s" }} />
                            </button>
